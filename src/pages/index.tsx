  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center justify-between w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:border-gray-400 dark:hover:border-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors min-w-0 text-gray-900 dark:text-white"
      >
        <div className="truncate flex-1">
          {selectedOption?.label || 'Select option'}
        </div>
        <svg 
          className={`w-4 h-4 ml-2 text-gray-500 dark:text-gray-400 transition-transform flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`}
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
...
const Dashboard: React.FC = () => {
  ...
  const { isConnected, disconnect } = useRealTimeMessages({
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                ...
                <span className="ml-2 inline-flex items-center">
                  <div className={`w-1.5 h-1.5 rounded-full mr-1 ${isConnected ? 'bg-green-500' : 'bg-red-500'}`} />
